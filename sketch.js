...
function pauseUnpause(pause) {
	paused = pause;
	runPauseButton.label = paused ? 'Bắt đầu' : 'Dừng';
}
...
function setup() {
	...
	runPauseButton = new Button('Bắt đầu', 430, 20, 100, 30, runpause);
	stepButton = new Button('Theo bước', 430, 70, 120, 30, step);
	rerunButton = new Button('Khởi tạo', 430, 120, 100, 30, restart);
	settingBox = new SettingBox(
		'Có đường chéo',
		430,
		180,
		allowDiagonals,
	...
	uiElements.push(rerunButton);
	uiElements.push(settingBox);
...
function searchStep() {
	if (!paused || stepsAllowed > 0) {
		...
		switch (result) {
			case -1:
				status = 'không tìm được đường đi!';
				logTimings();
				pauseUnpause(true);
				break;
			case 1:
				status = 'tìm được đường đi!';
				logTimings();
				pauseUnpause(true);
				break;
			case 0:
...
function draw() {
	...
	text('Trạng thái tìm kiếm: ' + status, 10, 480);
	textSize(20);
